//thanks for kocoafab, arduinogetstarted, 도매키트 아두몰
int position_pin[] = {1,2,3,4};               //4자리 결정 핀
int segment_pin[] = {5,6,7,8,9,10,11,12};     //세그먼트 제어 핀
const int delayTime = 5;                      //일시정지 시간
#define TRIG A3 //TRIG 핀 설정 (초음파 보내는 핀)
#define ECHO A4 //ECHO 핀 설정 (초음파 받는 핀)
...
void loop() {
  ...
  //thanks 도매키트 아두몰
  String du = duration
  String one = du.substring(0, 1)
  String two = du.substring(2, 2)
  String three = du.substring(3, 3)
  //thanks arduinogetstarted
  show(1,one);                //첫 번째 자리, 1출력
  delay(delayTime);         //0.005초 일시정지
  show(2,two);                //두 번째 자리, 2출력
  delay(delayTime);         //0.005초 일시정지
  show(3,three);                //세 번째 자리, 3출력
  delay(delayTime);         //0.005초 일시정지
  delay(delayTime);
